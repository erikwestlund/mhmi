...
- **Spread, not headcount, drives SEs:** The standard error of a slope depends on the spread of the predictor, not the raw row count. Because all imputed $X$ are identical for each value of E, they add almost no variance, so the denominator of $\mathrm{SE}(\hat\beta_X)$ barely changes. Worse, single-value imputation pretends those fills are certain, so standard errors can be *too* small.
...
- Rubin's rules still help by keeping uncertainty honest; we recover truth without pretending the imputed rows are certain

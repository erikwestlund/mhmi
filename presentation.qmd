...
- This presentation code be view on GitHub at [https://github.com/erikwestlund/mhmi](https://github.com/erikwestlund/mhmi)
- The presentation itself can be viewed at [https://erikwestlund.github.io/mhmi](https://erikwestlund.github.io/mhmi)
...
- I have material prepared, but if anyone has particular areas of concern, I can try to address them in my commentary where relevant.
...
- Don't let the teaching examples make you jump past important causal structure assumptions underpinning the discussed methods
...
# MAR: missingness driven only by an observed factor
dag_mar <- dagify(
  X ~ E,
  Y ~ X + E,
  X_miss ~ E,
...
# Outcome-dependent missingness (Y drives the recording of X)
dag_mar_y <- dagify(
  X ~ E,
  Y ~ X + E,
  X_miss ~ Y,
  coords = list(
    x = c(E = 0, X = 1.5, Y = 3, X_miss = 3.2),
    y = c(E = 1, X = 0,   Y = 0, X_miss = 1.7)
  ),
  labels = c(
    "E"      = "observed factor\n(e.g., education)",
    "X"      = "nutrition",
...
p_mar_y <- ggdag(dag_mar_y, text = TRUE, use_labels = "label") + theme_dag()
...
- Another variable `E` affects both nutrition and birthweight
- No missingness yet; this is the baseline, "omniscient" data-generating process.
...
# MCAR: Missing Completely At Random {data-background-color="#002D72"}
...
# MAR: Missing At Random {data-background-color="#002D72"}

## MAR: Nonresponse Driven by E

- Missingness rises with an observed factor (e.g., education, clinic context)
- Education affects both nutrition and birthweight.
- Dropping cases with missing values up-weights higher-education (for example) mothers, biasing the nutrition effect.
...
- Missingness depends on E (observed)
- All summaries and diagnostics in this MAR section restrict to those E-driven scenarios
- Next section: reuse the same data but let missingness depend on X to show why that mechanism becomes MNAR
...
mar_grid_mar <- mar_grid |>
  dplyr::filter(label == "Full data" | grepl("E-driven", label))

mar_grid_x <- mar_grid |>
  dplyr::filter(label == "Full data" | grepl("X-driven", label))

scenario_levels_mar <- mar_grid_mar$label
scenario_levels_x <- mar_grid_x$label

full_ref <- mar_grid_mar$data[[1]]
ref_Y <- mean(full_ref$Y); ref_X <- mean(full_ref$X); ref_E <- mean(full_ref$E)

mar_summary <- mar_grid_mar |>
  mutate(
  ...
  mutate(
    ...
    `E sd`   = round(sd_E, 2),
    scenario = factor(label, levels = scenario_levels_mar)
  ) |>
  arrange(scenario) |>
...
- Bias increased when relationship between E and p(missing) is stronger
...
- True $β_X$ = 0.50
- E affects both `Y`, `X`, and `p(x missing)`; these models are mis-specified without `E`
- MNAR and mis-specified models tend to go hand-in-hand -- you are missing key variables in your overall causal structure

```{r}
#| label: mar-scenarios-reg-naive
#| echo: false

order_levels <- scenario_levels_mar

mar_reg_naive <- purrr::map2_dfr(mar_grid_mar$data, mar_grid_mar$label, function(dat, lbl) {
  scen <- lbl
  if (scen == "Full data") {
    dat_use <- dat
...
- True $β_X$ = 0.50
- Even without any imputation, the adjusted model recovers $β_X$ across scenarios.
- The main consequence is power loss, not bias.

```{r}
#| label: mar-scenarios-reg
#| echo: false
mar_reg <- purrr::map2_dfr(mar_grid_mar$data, mar_grid_mar$label, function(dat, lbl) {
  scen <- lbl
  if (scen == "Full data") {
    dat_use <- dat
...
- If we model the variables that drive missingness (here, `E`), listwise deletion analyses stay near the truth.
- In that case, missing data mostly hurts precision: fewer cases translates to bigger SEs.
- Trouble begins when missingness of a variable depends on its own value: then MAR is violated relative to the analysis, and bias appears.
- In real workflows we can combine modeling plus imputation; this performs best.

## Common, Non-Ideal Imputation Tactics

- We apply four historically common tactics to each MAR scenario (E-driven at 10/30/60% missing) plus the full, no-missing baseline.
...
impute_results <- mar_grid_mar |>
  ...
  mutate(
    label = factor(label, levels = scenario_levels_mar),
...
- In this case, adding an indicator reduced bias somewhat compared to mean imputation, especially when missingness was lower, but bias remains
...
- Mechanics: sample with replacement from observed $X$'s, replace the missing Xs with thoes values, and analyze $Y \sim X + E$ on the filled in dataset.
...
- Works when missingness is MCAR; once it depends on $E$, donor pools misrepresent the missing cases.
...
## Regression Imputation: Mechanics
...
- Mechanics: use the observed $E$ to predict $X$ for every missing case, replace the missing Xs with that prediction, and then run the outcome regression.
...
## Regression Imputation: Mechanics (Multiple Missingness Causes)
...
- **Same line, more dots:** Regression imputation deterministically replaces each missing $X$ with its predicted value from $E$. Those rows reinforce the complete-case relationship instead of adding new leverage.
- **False confidence:**  You can see how SEs mirror the omniscient model, but they should in fact be larger than the omniscient model.

# Multiple Imputation

```{r}
#| label: mar-y-driven-data
#| include: false

set.seed(2026)
mar_y_data <- simulate_birth_data(
  n         = 6000,
  beta_x    = beta_x_true,
  beta_e    = beta_e_true,
  sd_error  = sd_error_true,
  ...
  mutate(
    ...
    p_miss_y = plogis(-0.3 - 1.1 * y_std),
    miss_y = rbinom(n(), 1, p_miss_y),
    X_marY = if_else(miss_y == 1, NA_real_, X)
  )

mar_y_full_fit <- lm(Y ~ X + E, data = mar_y_data)
mar_y_list_data <- mar_y_data |> filter(!is.na(X_marY))
mar_y_list_fit <- lm(Y ~ X_marY + E, data = mar_y_list_data)

mar_y_imp <- mar_y_data |>
  transmute(Y, E, X_obs = X_marY)

pred_y <- mice::make.predictorMatrix(mar_y_imp)
pred_y[,] <- 0
pred_y["X_obs", c("Y", "E")] <- 1

mar_y_mi <- mice::mice(
  mar_y_imp,
  m = 15,
  method = "pmm",
  predictorMatrix = pred_y,
  seed = 2027,
  printFlag = FALSE
)

mar_y_mi_fit <- with(mar_y_mi, lm(Y ~ X_obs + E))
mar_y_mi_pool <- mice::pool(mar_y_mi_fit)
mar_y_mi_row <- summary(mar_y_mi_pool) |> dplyr::filter(term == "X_obs")

mar_y_compare <- tibble(
  Method = c("Full data (truth)", "Listwise deletion", "Multiple imputation"),
  beta_x = c(
    coef(mar_y_full_fit)[["X"]],
    coef(mar_y_list_fit)[["X_marY"]],
    mar_y_mi_row$estimate
  ),
  se = c(
    coef(summary(mar_y_full_fit))["X", "Std. Error"],
    coef(summary(mar_y_list_fit))["X_marY", "Std. Error"],
    mar_y_mi_row$std.error
  ),
  bias = beta_x - beta_x_true
)

mar_y_missing_rate <- mean(mar_y_data$miss_y)
mar_y_n_list <- nrow(mar_y_list_data)
```
...
mar_summary_x <- mar_grid_x |>
  ...
  mutate(
    ...
    `E mean` = round(mean_E, 2),
    `E sd`   = round(sd_E, 2),
    scenario = factor(label, levels = scenario_levels_x)
  ) |>
  arrange(scenario) |>
  mutate(scenario = as.character(scenario)) |>
  select(
...
mar_reg_naive_x <- purrr::map2_dfr(mar_grid_x$data, mar_grid_x$label, function(dat, lbl) {
  scen <- lbl
  if (scen == "Full data") {
    dat_use <- dat
    fit <- lm(Y ~ X, data = dat_use)
  ...
  mutate(
    order = match(scenario, scenario_levels_x),
    est_X = round(est_X, 3),
    se_X  = round(se_X, 3)
  ) |>
  arrange(order) |>
...
mar_reg_x <- purrr::map2_dfr(mar_grid_x$data, mar_grid_x$label, function(dat, lbl) {
  scen <- lbl
  if (scen == "Full data") {
    dat_use <- dat
    fit <- lm(Y ~ X + E, data = dat_use)
...
missing_lookup_x <- mar_grid_x |>
  mutate(missing_prop = purrr::map_dbl(data, ~ mean(is.na(.x$X_mar)))) |>
  select(label, missing_prop)

listwise_lookup_x <- mar_reg_x |>
  transmute(label = scenario, N_listwise = N, beta_listwise = est_X, se_listwise = se_X)

mar_mi_results_x <- mar_grid_x |>
  mutate(mi = purrr::map(data, ~ run_mi(.x, m = 10))) |>
  mutate(
    mi_est = purrr::map_dbl(mi, "estimate"),
    mi_se = purrr::map_dbl(mi, "se")
  ...
  select(-mi) |>
  left_join(missing_lookup_x, by = c("label" = "label")) |>
  left_join(listwise_lookup_x, by = c("label" = "label")) |>
  mutate(
    scenario = factor(label, levels = scenario_levels_x),
    Missing = paste0(round(missing_prop * 100), "%"),
    mi_bias = mi_est - beta_x_true
...
mar_mi_table_x <- mar_mi_results_x |>
  mutate(
    `N listwise` = N_listwise,
    `β_X listwise` = round(beta_listwise, 3),
    `SE listwise` = round(se_listwise, 3),
...
# MAR, MI, &amp; Outcome Variables

## Common Mistake: Not Imputing Outcomes

- When missingness depends on `Y`, analysts often hesitate to use `Y` to impute `X` 
- But if you leave `Y` out you replicate the listwise bias because the imputations can't see the selection mechanism. 

## “Isn’t $Y$ Later in Time?”

- Yes. Nutrition is recorded before birthweight, but by the time we analyze the data, $Y$ is observed for every mother.
- Imputation models the joint distribution of $(X, Y, E)$ using all observed information; it doesn’t send signals back in time.
- Leaving $Y$ out because of timing hides the mechanism that made $X$ go missing and recreates the listwise bias.
- The causal analysis still honors temporal order: first repair the dataset using all available information, then fit $Y \sim X + E$ on the completed data.

## Outcome-Dependent Missingness: New Scenario

- In this new scenario, nutrition still causes birthweight, but now **recording nutrition depends on the outcome itself**: emergency low-birthweight deliveries lose the time needed to capture $X$.
- Missingness is MAR because it relies on observed $Y$ and $E$, yet complete-case regression **drops precisely the low-$Y$ mothers** and tilts $\hat\beta_X$.
- Multiple imputation repairs the bias by letting $Y$ (and $E$) predict the missing $X$ values before fitting the usual $Y \sim X + E$ model.


## DAG: $Y$ Drives Whether $X$ Is Observed
...
- $n = 6{,}000$ mothers; true $\beta_X = 0.50$
- About `r scales::percent(mar_y_missing_rate, accuracy = 0)` of nutrition scores are missing when birthweight is low
- Listwise deletion keeps only `r mar_y_n_list` observations and overstates the nutrition slope

## Listwise vs. MI

```{r}
#| label: mar-y-compare-table
#| echo: false

mar_y_display <- mar_y_compare |>
  mutate(
    beta_x = round(beta_x, 3),
    se = round(se, 3),
    bias = round(bias, 3)
...
if (requireNamespace("kableExtra", quietly = TRUE)) {
  mar_y_display |>
    kableExtra::kable(format = "html",
                      col.names = c("Method", "$\\hat{\\beta}_X$", "SE", "Bias")) |>
    kableExtra::kable_styling(font_size = 13) |>
    kableExtra::column_spec(2, bold = TRUE) |>
    kableExtra::column_spec(4, color = "red")
} else {
  knitr::kable(mar_y_display,
               col.names = c("Method", "$\\hat{\\beta}_X$", "SE", "Bias"),
               format = "simple")
}
```

- **Listwise deletion:** drops low-$Y$ deliveries, depressing the slope of `X`
- **Multiple imputation:** models $X$ with predictors $(Y,E)$, fills 15 completed datasets with predictive mean matching, and Rubin’s rules recover the true slope and honest SEs.
- The outcome still depends on the same covariates ($X$ and $E$); MI just leverages *outcome information* during imputation instead of discarding it via listwise deletion.

# MI: Summary

## Multiple Imputation: Takeaways

- MI lands on the true $\beta_X$ even when missingness depends on E, provided we include the variables (here, $E$) that explain that missingness.
...
# MNAR-lite: Missingness Responds to X {data-background-color="#002D72"}

## MNAR-lite Setup

- Same 20,000-mother dataset as before
- Missingness now rises when nutrition is *low* even after we condition on E; missingness depends on X.
- Analysts observe $Y$, $E$, and an $X$ that is sometimes missing. Because the chance of missingness rises when the *true* $X$ is low, the mechanism is technically MNAR.

## Why OLS Survives This Scenario

- The missingness in the DAG depends only on the regressors ($X$ and $E$), not on any unmodeled part of $Y$.
- That means the complete cases still satisfy the correct outcome relationship: $E[Y \mid X, E] = \beta_X X + \beta_E E$.
- Translation: even with MNAR missingness on $X$, listwise deletion remains unbiased **as long as** the outcome model includes the variables driving the missingness. You lose precision, not correctness.

## MNAR-lite: Selection Snapshot

- Education means stay similar, but the observed nutrition mean inflates as missingness rises
- These differences confirm the selection is MNAR relative to nutrition, even though our regression can still recover the truth with the right control set

```{r}
#| label: mar-x-summary-table
#| echo: false
...
if (requireNamespace("kableExtra", quietly = TRUE)) {
  mar_summary_x |>
    kableExtra::kable(format = "html") |>
    kableExtra::kable_styling(font_size = 14) |>
    kableExtra::column_spec(6, bold = TRUE, color = "red")
} else {
  knitr::kable(mar_summary_x, format = "simple")
}
```

## MNAR-lite: Naive Regression (Y ~ X)

- Dropping rows with missing $X$ and ignoring $E$ yields the expected MNAR bias
- Higher missingness exaggerates the up-weighting of well-nourished mothers, so slopes drift up

```{r}
#| label: mar-x-reg-naive-table
#| echo: false
...
if (requireNamespace("kableExtra", quietly = TRUE)) {
  ...
  knitr::kable(mar_reg_naive_x,
               col.names = c("Scenario","N","β_X","SE(β_X)"),
               format = "simple")
}
```

## MNAR-lite: Adjusted Regression (Y ~ X + E)

- Conditioning on $E$ blocks the backdoor path, so even listwise deletion stays near the true $\beta_X = 0.50$
- The only cost is power loss; SEs grow as missingness increases

```{r}
#| label: mar-x-reg-table
#| echo: false
...
if (requireNamespace("kableExtra", quietly = TRUE)) {
  mar_reg_x |>
    kableExtra::kable(format = "html",
                      col.names = c("Scenario","N","β_X","SE(β_X)","β_E","SE(β_E)")) |>
    kableExtra::kable_styling(font_size = 14) |>
    kableExtra::column_spec(3, bold = TRUE, color = "steelblue") |>
    kableExtra::column_spec(5, color = "steelblue")
} else {
  knitr::kable(mar_reg_x,
               col.names = c("Scenario","N","β_X","SE(β_X)","β_E","SE(β_E)"),
               format = "simple")
}
```

## MNAR-lite: Listwise vs. MI

- MI lines up with listwise deletion because both models respect the same ignorability structure
- Rubin's rules still help by keeping 

```{r}
#| label: mar-x-mi-table
#| echo: false
...
if (requireNamespace("kableExtra", quietly = TRUE)) {
  mar_mi_table_x |>
    kableExtra::kable(format = "html") |>
    kableExtra::kable_styling(font_size = 13) |>
    kableExtra::column_spec(4, color = "steelblue") |>
    kableExtra::column_spec(6, bold = TRUE, color = "darkgreen")
} else {
  knitr::kable(mar_mi_table_x, format = "simple")
}
```

# MNAR: Missing Not At Random {data-background-color="#002D72"}
...
- Let's finish by examining such a scenario
...
- Let's compare listwise deletion vs. MI when we have a complex causal graph with unobserved data
...
- The MNAR-lite illustration showed how listwise deletion recovered accurate model estimates, but our Table 1 is still wrong
- We can see here by using MI, our summary statistics are more accurate than listwise deletion, but still biased.
...
# Conclusion {data-background-color="#002D72"}
